...
```mermaid
flowchart TD
    Client((Client / Attacker\nInternet)) -->|Gởi gói tin TCP| Kernel[Tầng Vật lý Linux Kernel\n(nftables)]
    
    %% Tầng Kernel Firewall
    Kernel -->|IP bị Permanent Ban?| DropKernel[DROP tức khắc\nZero-latency & No CPU/RAM rác]
    Kernel -->|Lỗi trạng thái / SYN Flood?| DropKernel
    Kernel -->|IP đã ở trong Whitelist?| FastTrack{Làn ưu tiên\nBypass toàn bộ logic}
    
    %% Tầng Proxy Application
    Kernel -->|Gói tin có vẻ hợp lệ| Proxy[Tầng Ứng dụng Proxy\nRust Tokio Async]
    
    Proxy -->|Vượt Max Conns / IP?| TempDrop[Đóng kết nối tạm thời\nRate Limit]
    Proxy -->|Spam vội nhiều / 1 phút?| Strike[Cấp thẻ phạt Strike\nGiam vào Blacklist tạm thời]
    Strike --> TempDrop
    Proxy -->|Dính đủ số thẻ phạt?| PermanentBan[Khóa vĩnh viễn IP\nBáo cáo thẳng xuống Kernel]
    
    Proxy -->|Thỏa mãn mọi điều kiện| StreamProxy[Cho phép\nMở luồng chuyển đổi Data]
    
    %% Tự động Whitelist
    StreamProxy -.->|Duy trì kết nối tốt > 30s| AddWhitelist[Tự động nhận diện Player\nThêm IP thẳng vào Whitelist Kernel]
    AddWhitelist -.-> FastTrack
    
    FastTrack --> Backend
    StreamProxy <==> Backend[(Game Server / DB\nBackend Targets)]
```
...
```mermaid
flowchart LR
    Internet((Internet\nKẻ tấn công / Người chơi)) --> NIC[Card Mạng\n(NIC Hardware)]
    
    subgraph Tầng Hệ Điều Hành (Kernel Space)
        NIC --> XDP{XDP / eBPF\n(Tương lai nâng cấp)}
        XDP -.->|Drop tại Driver| XDPDrop[Hủy vô hình\nKhông tốn RAM]
        XDP -->|Pass| NetStack[Linux Network Stack\nCấp phát sk_buff]
        
        NetStack --> nftables{nftables Firewall\nLớp bảo vệ 1}
        nftables -->|Permanent Ban\nLỗi State| KernelDrop[Hủy ngay lập tức]
    end
    
    subgraph Tầng Ứng Dụng (User Space)
        nftables -->|Hợp lệ| RustProxy{Rust Proxy Tokio\nLớp bảo vệ 2}
        RustProxy -->|Vượt Rate Limit\nĂn Strike| AppDrop[Từ chối kết nối]
