...
Dưới đây là mô hình giải thích luồng đi của một kết nối từ ngoài Internet đi qua hệ thống Proxy bảo vệ trước khi rớt tới khu vực máy chủ Game đích:

```mermaid
flowchart TD
    Client(("Client / Attacker<br>Internet")) -->|Gởi gói tin TCP| Kernel["Tầng Vật lý Linux Kernel<br>(nftables)"]
    
    %% Tầng Kernel Firewall
    Kernel -->|IP bị Permanent Ban?| DropKernel["DROP tức khắc<br>Zero-latency & No CPU/RAM rác"]
    Kernel -->|Lỗi trạng thái / SYN Flood?| DropKernel
    Kernel -->|IP đã ở trong Whitelist?| FastTrack{"Làn ưu tiên<br>Bypass toàn bộ logic"}
    
    %% Tầng Proxy Application
    Kernel -->|Gói tin có vẻ hợp lệ| Proxy["Tầng Ứng dụng Proxy<br>Rust Tokio Async"]
    
    Proxy -->|Vượt Max Conns / IP?| TempDrop["Đóng kết nối tạm thời<br>Rate Limit"]
    Proxy -->|Spam vội nhiều / 1 phút?| Strike["Cấp thẻ phạt Strike<br>Giam vào Blacklist tạm thời"]
    Strike --> TempDrop
    Proxy -->|Dính đủ số thẻ phạt?| PermanentBan["Khóa vĩnh viễn IP<br>Báo cáo thẳng xuống Kernel"]
    
    Proxy -->|Thỏa mãn mọi điều kiện| StreamProxy["Cho phép<br>Mở luồng chuyển đổi Data"]
    
    %% Tự động Whitelist
    StreamProxy -.->|Duy trì kết nối tốt > 30s| AddWhitelist["Tự động nhận diện Player<br>Thêm IP thẳng vào Whitelist Kernel"]
    AddWhitelist -.-> FastTrack
    
    FastTrack --> Backend
    StreamProxy <==> Backend[("Game Server / DB<br>Backend Targets")]
```

### Chặng đường của gói tin (Packet Network Stack)
Để hiểu rõ mức độ bọc lót của hệ thống, đây là sơ đồ mô tả cách một gói tin di chuyển từ môi trường mạng vật lý vượt qua các tầng của hệ điều hành Linux:

```mermaid
flowchart LR
    Internet(("Internet<br>Kẻ tấn công / Người chơi")) --> NIC["Card Mạng<br>(NIC Hardware)"]
    
    subgraph KernelSpace [Tầng Hệ Điều Hành Mức Kernel]
        NIC --> XDP{"XDP / eBPF<br>(Tương lai nâng cấp)"}
        XDP -.->|Drop tại Driver| XDPDrop["Hủy vô hình<br>Không tốn RAM"]
        XDP -->|Pass| NetStack["Linux Network Stack<br>Cấp phát sk_buff"]
        
        NetStack --> nftables{"nftables Firewall<br>Lớp bảo vệ 1"}
        nftables -->|Permanent Ban / Lỗi State| KernelDrop["Hủy ngay lập tức"]
    end
    
    subgraph UserSpace [Tầng Ứng Dụng Mức User Space]
        nftables -->|Hợp lệ| RustProxy{"Rust Proxy Tokio<br>Lớp bảo vệ 2"}
        RustProxy -->|Vượt Rate Limit / Ăn Strike| AppDrop["Từ chối kết nối"]
        RustProxy -->|Hợp lệ| Target[("Game Server Đích")]
    end
```

### 1. Bảo vệ hai lớp (Dual-Layer Protection)
...
### 2. Hệ thống thẻ phạt thông minh (Intelligent Strike System)
